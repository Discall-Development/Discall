...
function createPacket(key, data, param, reason) {
    console.log(key);
    let url = formatUrl(types_1.HttpUri[key], data);
    let result = {
        uri(base) {
            base.pathname += url;
            return {
...
async function send(packet) {
    ...
    if (uri) {
        let headers = new runtimeModule_1.fetch.Headers({
...
async function toFormData(data) {
    let body = new runtimeModule_1.fetch.FormData();
    let filenames = data.attachments;
    body.append("payload_json", JSON.stringify(data));
    for (const idx in filenames) {
        let filename = filenames[idx].filename;
        let data = await promises_1.default.readFile(filename);
